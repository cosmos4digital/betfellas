-- ============================================================
-- KUPA GURU - TAM Kurulum SQL'i (v1 + v2 birleşik)
-- Dünya Kupası 2026 - Arkadaşlar Arası Sanal Tahmin Oyunu
-- TEMİZ bir veritabanında tek seferde çalıştırın.
-- (Daha önce kurulum yaptıysan önce temizlik script'ini çalıştır.)
-- ============================================================
...
create table public.profiles (
  ...
  avatar_url  text,
  frame       text check (frame in ('neon','gold','fire')), -- kozmetik çerçeve (mağaza)
...
create table public.matches (
  id          uuid primary key default gen_random_uuid(),
  external_id text unique,            -- API-Football fixture id (gerçek maç senkronu)
  team1       text not null,
  team2       text not null,
  score1      integer,                -- canlı / biten skor
  score2      integer,
...
create index idx_ad_views_daily on public.ad_views(user_id, league_id, created_at);
...
-- ---------- 9. RPC: REKLAM ÖDÜLÜ AL (günde 5 hak, 50 GP) ----------
...
-- ---------- 10. RPC: ÇERÇEVE SATIN AL (GP ile, kozmetik) ----------
...
-- ad_views: herkes sadece kendi izlemelerini görür; yazma sadece RPC üzerinden
...
-- ---------- 13. ÖRNEK DK2026 MAÇLARI (test için) ----------
